...
window.addEventListener("load", function () {
    ...
    document.getElementById("najblizjiHotel")
        .addEventListener("click", function () {
            alert(
                "Najbližji hotel je: " + najblizjiHotelNaziv + ", " + mesto +
...
function prikaziHoteleZnotrajRadija() {
    ...
    preveriPogojeSklopaHoteli(function () {
        ...
        var lon1 = parki[kandidatiParkov[0]].koordinate.lng;
        var lat2 = parki[kandidatiParkov[1]].koordinate.lat;
        var lon2 = parki[kandidatiParkov[1]].koordinate.lng;
        var srednjaTocka = middlePoint(lat1, lon1, lat2, lon2);
        ...
        pridobiPodatke("hoteli", function (jsonRezultat, vrstaInteresneTocke) {
            ...
            var radij = document.querySelector("#radij").value;
            var opcii = {
                radius: radij * 1000,
                color: "#ff0000",
                stroke: false,
            };
            radijHotelov = L.circle(srednjaTocka, opcii);
            mapa.addLayer(radijHotelov);

            // Poišči vse hotele znotraj radija
            izracunajVseHoteleVRadiju(srednjaTocka,radij);
...
function izracunajVseHoteleVRadiju(lat, lng, radij) {
    ...
    radij = lng;
    lng = lat.toString().slice(18,35);
    lat = lat.toString().slice(0,17);
    ...
    for (let i = 0; i < hoteli.length; i++) {
        ...
        if(oddaljenost < najblizjiHotelOddaljenost){
            ...
            najblizjiHotelNaziv = hoteli[i].lastnosti.ime;
            mesto = hoteli[i].lastnosti.mesto;
        }
...
function preveriPogojeSklopaHoteli(callback) {
    var sporocilo = "Skupni znesek v osnovnih podatkih mora biti več kot 0 €!";
    var sporocilo1 = "Izbrati je potrebno 2 parka!";
    var ali = document.getElementsByClassName("vsota")[0].innerHTML == 0;
    var ali1 = kandidatiParkov.length != 2;

    if(ali && ali1){
        alert(sporocilo + " " + sporocilo1);
    }
    else {
        if (!ali) {
            callback();
        } else alert(sporocilo);

        if (!ali1) {
            callback();
        } else alert(sporocilo1);
    }
...
function dodajParkMedKandidate(indeks) {
    ...
    if (!kandidatiParkov.includes(indeks) && kandidatiParkov.length < 2) {
        ...
        stolpec1.innerHTML = "<b>"+ parki[indeks].lastnosti.ime.toString() +"<b style='font-weight: normal'> z oceno </b>"+ parki[indeks].lastnosti.ocena.toString() + "</b>";
        stolpec2.innerHTML = "<button class='rob senca' onclick='odstraniKotKandidata(this, " +  indeks + ")'>" +
